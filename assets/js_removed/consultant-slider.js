  ...
  jQuery(document).ready(function($) {
    (function(wd) {
      ...
      wd.init = function(isMobile) {
        ...
        if (
          isMobile && (typeof wd.config.enableMobile == 'undefined' || !parseInt(wd.config.enableMobile))
        ) {
        ...
        if (isMobile) {
          ...
          var widthSlides = function() {
            sliderWidth = $widget.innerWidth() - parseInt($widget.css('padding-left')) - parseInt($widget.css('padding-right'));
        ...
        if (
          typeof wd.config.gradient != 'undefined' && parseInt(wd.config.gradient) && typeof wd.config.gradientG != 'undefined' && typeof wd.config.gradientC1 != 'undefined' && typeof wd.config.gradientC2 != 'undefined'
        ) {
          var val = 'linear-gradient(' + parseInt(wd.config.gradientG) + 'deg,' + wd.config.gradientC1 + ',' + wd.config.gradientC2 + ')';
          if (isMobile) {
      ...
      wd.stat.showWidget = function() {
        if (typeof wd.config.isTest !== 'undefined' && wd.config.isTest)
          return;
        $.post(wd.statUrl, {
          'idpartner': wd.config.idpartner,
          'siteId': wd.config.siteId,
          'act': 'showWidget'
        });
      };
      ...
      $('form#remont', $widget).submit(function() {
        ...
        d.push({
          ...
          'value': wd.messageCustomer.trim()
        });
        d.push({
          'name': 'idpartner',
          'value': wd.config.idpartner
        });
        d.push({
          'name': 'siteId',
          'value': wd.config.siteId
        });
        d.push({
          'name': 'isMobile',
      ...
      wd.scrollWidget = function() {
        ...
        if ($contentWrap.height() < chHeight) {
          $contentWrap.animate({
            scrollTop: chHeight - $contentWrap.height()
          });
        }
      };
      ...
      function addMessWithProcess(mes, type, callback) {
        if (statusWriteMessage) {
          aMessageQueue[aMessageQueue.length] = {
            'message': mes,
            'type': type,
            'callback': callback
          };
      ...
      window._gr_reRenderTimeMessage = function() {
        $contentWrap.find('.date').each(function(i, e) {
          ...
          if (diff >= 360000) {
            var h =
              $e.text('Отправлено в ' + (elTime.getHours() < 10 ? '0' + elTime.getHours() : elTime.getHours()) + ':' + (elTime.getMinutes() < 10 ? '0' + elTime.getMinutes() : elTime.getMinutes()));
            return;
          }
      ...
      function sliderGoSlide(route) {
        ...
        $rubr.animate({
          'margin-left': -(currentSlide - 1) * sliderWidth + 'px'
        }, 200);
