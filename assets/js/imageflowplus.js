...
function flowplus(instance) {
  ...
  this.refresh = function(onload) {
    ...
    this.flowplus_div.onmouseover = function() {
      thisObject.autorotate = 'off';
      return false;
    };
    this.flowplus_div.onmouseout = function() {
      thisObject.autorotate = thisObject.autoRotate;
      return false;
    };
    ...
    this.slider_div.onmousedown = function() {
      thisObject.dragstart(this);
      return false;
    };
    ...
    for (var index = 0; index < this.max; index++) {
      ...
      if ((image_block.nodeType == 1) && (image_block.nodeName != "NOSCRIPT")) {
        ...
        if (image.getAttribute("data-style") && (image.getAttribute("data-style") == 'wpif2_lightbox')) {
          ...
          image_block.ondblclick = function() {
            thisObject.autoRotate = 'off';
            thisObject.showTop(this);
            return false;
          }
  ...
  this.Touch = {
    ...
    touch_init: function() {
      thisObject.slider_div.ontouchstart = function(event) {
        thisObject.Touch.touch_start(event, thisObject.ifp_sliderdiv);
        return false;
      };
      thisObject.slider_div.ontouchend = function(event) {
        thisObject.Touch.touch_end(event);
        return false;
      };
      thisObject.slider_div.ontouchmove = function(event) {
        thisObject.Touch.touch_move(event);
        return false;
      };
      thisObject.slider_div.ontouchcancel = function(event) {
        thisObject.Touch.touch_cancel(event);
        return false;
      };

      thisObject.imgs_div.ontouchstart = function(event) {
        thisObject.Touch.touch_start(event, thisObject.ifp_imagesdiv);
        return false;
      };
      thisObject.imgs_div.ontouchend = function(event) {
        thisObject.Touch.touch_end(event);
        return false;
      };
      thisObject.imgs_div.ontouchmove = function(event) {
        thisObject.Touch.touch_move(event);
        return false;
      };
      thisObject.imgs_div.ontouchcancel = function(event) {
        thisObject.Touch.touch_cancel(event);
        return false;
      };

      topbox_div = document.getElementById(thisObject.ifp_topboxdiv);
      topbox_div.ontouchstart = function(event) {
        thisObject.Touch.touch_start(event, thisObject.ifp_topboxdiv);
        return false;
      };
      topbox_div.ontouchend = function(event) {
        thisObject.Touch.touch_end(event);
        return false;
      };
      //topbox_div.ontouchmove = function(event) { thisObject.Touch.touch_move(event); return false; };
      topbox_div.ontouchcancel = function(event) {
        thisObject.Touch.touch_cancel(event);
        return false;
      };
    },
  ...
  this.showTop = function(image_block) {
    ...
    var t = new Image();
    t.onload = (function() {
      thisObject.showImg(image_block, t, t.width, t.height);
    });
  ...
  this.showImg = function(image_block, img, img_width, img_height) {
    ...
    if (img_width == 0 || img_height == 0) {
      img.onload = (function() {
        thisObject.showImg(image_block, img, img.width, img.height);
      });
      return;
    }
      ...
      for (index = image_block.i - 1; index >= 0; index--) {
        ...
        if (prev_image.getAttribute("data-style") && (prev_image.getAttribute("data-style") == 'wpif2_lightbox')) {
          ...
          prev_div.onclick = (function() {
            thisObject.showImg(prev_image_block, p, p.width, p.height);
            return false;
          });
      ...
      for (index = image_block.i + 1; index < this.max; index++) {
        ...
        if (next_image.getAttribute("data-style") && (next_image.getAttribute("data-style") == 'wpif2_lightbox')) {
          ...
          next_div.onclick = (function() {
            thisObject.showImg(next_image_block, n, n.width, n.height);
            return false;
          });
